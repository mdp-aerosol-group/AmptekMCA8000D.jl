...
function __init__()
    ...
    def packmsg(header, data):
        ...
        length = len(data)
        ba = header + packint(length) + bytearray(data, encoding = 'utf8')
        cs = chksum(ba)
    ...
    class device:
        ...
        def reqHWConfig(self):
            ...
            strg = lcfgstr[0].decode()
            cfg = {}
            for param in strg.split(';'):
                pv = param.split('=')
                if len(pv) == 2:
                    cfg.setdefault(pv[0], pv[1])
            return(cfg)
...
device() = py"device"()
printStatus(status) = py"printStatus"(status)
